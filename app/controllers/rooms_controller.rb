class RoomsController < ApplicationController
  ...
  def show
    ...
    @available_to_pick_players = PickedPlayer.where(user_id: nil).where("release_time <= ?", Time.zone.now.strftime('%Y-%m-%d %H:%M:%S')).order("release_time ASC").group_by(&:release_time)
